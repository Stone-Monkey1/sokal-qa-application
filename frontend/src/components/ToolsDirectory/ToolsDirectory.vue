    ...
    <div class="panel-group" id="toolAccordion">
      <div v-for="tool in filteredTools" :key="tool.id" class="accordion">
        ...
        <div v-if="openAccordions.includes(tool.id)" class="accordion-content">
          ...
          <div class="location-tags" v-if="tool.locationTags">
            <h4>Site locations:</h4>
            <span>{{ tool.locationTags.siteLocations?.join(', ') || 'N/A' }}</span><br />
            <strong>Web Dash locations:</strong>
            <span>{{ tool.locationTags.webDashLocations?.join(', ') || 'N/A' }}</span>
          </div>
          
          <div class="ticket-references" v-if="tool.ticketReferences.length">
            <h4>Tickets:</h4>
            <ul>
